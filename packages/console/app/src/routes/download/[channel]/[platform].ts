...
const downloadNames: Record<string, string> = {
  "darwin-aarch64-dmg": "Smartie Desktop.dmg",
  "darwin-x64-dmg": "Smartie Desktop.dmg",
  "windows-x64-nsis": "Smartie Desktop Installer.exe",
...
export async function GET({ params: { platform, channel } }: APIEvent) {
  ...
  const resp = await fetch(
    `https://github.com/anomalyco/${channel === "stable" ? "smartie" : "opencode-beta"}/releases/latest/download/${assetName}`,
    {
      cf: {
