...
const CLI_INSTALL_DIR = ".smartie/bin"
const CLI_BINARY_NAME = "smartie"
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    SMARTIE_SERVER_USERNAME: "smartie",
    SMARTIE_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    SMARTIE_EXPERIMENTAL_ICON_DISCOVERY: "true",
    SMARTIE_EXPERIMENTAL_FILEWATCHER: "true",
    SMARTIE_CLIENT: "desktop",
