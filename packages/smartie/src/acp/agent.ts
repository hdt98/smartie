...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@smartie-code/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `smartie auth login` in the terminal",
        name: "Login with smartie",
        id: "smartie-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Smartie stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const smartieProvider = providers.find((p) => p.id === "smartie")
    if (smartieProvider) {
      if (smartieProvider.models["big-pickle"]) {
        return { providerID: "smartie", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(smartieProvider.models))
      if (best) {
        return {
