...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run smartie",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for smartie session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found smartie session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["smartie", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting smartie...")
        UI.println()

        // Launch smartie TUI with session ID if available
        const { spawn } = await import("child_process")
        const smartieArgs = sessionId ? ["-s", sessionId] : []
        const smartieProcess = spawn("smartie", smartieArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          smartieProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`smartie exited with code ${code}`))
          })
          smartieProcess.on("error", reject)
        })
      },
    })
  },
