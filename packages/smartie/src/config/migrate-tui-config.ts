...
const TUI_SCHEMA_URL = "https://smartie.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from smartie.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const smartie = await smartieFiles(input)
  for (const file of smartie) {
...
async function smartieFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.SMARTIE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("smartie", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "smartie")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "smartie"))
  }
  if (Flag.SMARTIE_CONFIG) files.push(Flag.SMARTIE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "smartie"))
