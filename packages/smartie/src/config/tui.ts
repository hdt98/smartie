...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.SMARTIE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.SMARTIE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.SMARTIE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".smartie") && dir !== Flag.SMARTIE_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old smartie.json shape) still get their settings applied.
    const normalized = (() => {
