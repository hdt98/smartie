...
import { fn } from "@smartie-code/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Filesystem.readText(path.join(dotgit, "smartie"))
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.SMARTIE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            await Filesystem.write(path.join(dotgit, "smartie"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.SMARTIE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
