...
const managedConfigDir = process.env.SMARTIE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "smartie.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "smartie.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://smartie.ai/config.json",
      snapshot: false,
    })
...
test("ignores legacy tui keys in smartie config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://smartie.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "smartie.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "smartie.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .smartie directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const smartieDir = path.join(dir, ".smartie")
      await fs.mkdir(smartieDir, { recursive: true })
      const agentDir = path.join(smartieDir, "agent")
...
test("loads agents from .smartie/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const smartieDir = path.join(dir, ".smartie")
      await fs.mkdir(smartieDir, { recursive: true })

      const agentsDir = path.join(smartieDir, "agents")
...
test("loads commands from .smartie/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const smartieDir = path.join(dir, ".smartie")
      await fs.mkdir(smartieDir, { recursive: true })

      const commandDir = path.join(smartieDir, "command")
...
test("loads commands from .smartie/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const smartieDir = path.join(dir, ".smartie")
      await fs.mkdir(smartieDir, { recursive: true })

      const commandsDir = path.join(smartieDir, "commands")
...
test("does not try to install dependencies in read-only SMARTIE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.SMARTIE_CONFIG_DIR
  process.env.SMARTIE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.SMARTIE_CONFIG_DIR
    else process.env.SMARTIE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable SMARTIE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "smartie.json"),
        JSON.stringify({ $schema: "https://smartie.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "smartie.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .smartie config
      const projectDir = path.join(dir, "project")
      const smartieDir = path.join(projectDir, ".smartie")
      await fs.mkdir(smartieDir, { recursive: true })
      ...
      // Local .smartie config with different plugins
      await Filesystem.write(
        path.join(smartieDir, "smartie.json"),
        JSON.stringify({
          $schema: "https://smartie.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .smartie config with some overlapping plugins
      await Filesystem.write(
        path.join(smartieDir, "smartie.json"),
        JSON.stringify({
          $schema: "https://smartie.ai/config.json",
...
// Note: preload.ts sets SMARTIE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .smartie config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .smartie directory config enables it
      const smartieDir = path.join(dir, ".smartie")
      await fs.mkdir(smartieDir, { recursive: true })
      await Filesystem.write(
        path.join(smartieDir, "smartie.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/smartie")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/smartie")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-smartie@2.4.3")).toBe("oh-my-smartie")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@smartie/plugin@2.0.0")).toBe("@smartie/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-smartie@2.4.3", "file:///project/.smartie/plugin/oh-my-smartie.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.smartie/plugin/oh-my-smartie.js")
  })
  ...
  test("local plugin directory overrides global smartie.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const smartieDir = path.join(projectDir, ".smartie")
        const pluginDir = path.join(smartieDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("SMARTIE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["SMARTIE_DISABLE_PROJECT_CONFIG"]
    process.env["SMARTIE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["SMARTIE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["SMARTIE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .smartie/ directories when flag is set", async () => {
    const originalEnv = process.env["SMARTIE_DISABLE_PROJECT_CONFIG"]
    process.env["SMARTIE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .smartie directory with a command
          const smartieDir = path.join(dir, ".smartie", "command")
          await fs.mkdir(smartieDir, { recursive: true })
          await Filesystem.write(path.join(smartieDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .smartie should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["SMARTIE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SMARTIE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["SMARTIE_CONFIG_DIR"]
      process.env["SMARTIE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["SMARTIE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["SMARTIE_CONFIG_DIR"]
      } else {
        process.env["SMARTIE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("SMARTIE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["SMARTIE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SMARTIE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["SMARTIE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["SMARTIE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from SMARTIE_CONFIG_DIR, not project
...
describe("SMARTIE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in SMARTIE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SMARTIE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["SMARTIE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://smartie.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["SMARTIE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["SMARTIE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in SMARTIE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SMARTIE_CONFIG_CONTENT"]
